...
class LoginTest {
  ...
  void login_shouldAuthorizeUser_whenValidCredentials(String roleName, String credentialType)
      throws Exception {
    User user = $db.user().seed(1).role(getRole(roleName)).gender(gender).ageGroup(ageGroup).get();
  ...
  void login_shouldThrowException_whenInvalidCredentials() throws Exception {
    User user = $db.user().seed(1).role(adminRole).gender(gender).ageGroup(ageGroup).get();
    LoginRequestDto dto = $dto.loginRequestDto().seed(2).withUsername().get();
